...
def load_profile():
    """config/profile.yaml → dict. NFR-01: 최은희 지도사가 직접 편집하는 설정.
    pyyaml 이 없으면 빌드 전체를 막지 않고 빈 프로필로 넘어간다 — 대시보드는 여전히 열려야 한다."""
    path = os.path.join(CFG, 'profile.yaml')
    if not os.path.exists(path):
        print(f"  ! 없음: {path} (프로필 탭이 비어 보입니다)")
        return {}
    try:
        import yaml
    except ImportError:
        print("  ! pyyaml 미설치 — 'pip install pyyaml' 후 다시 빌드하면 프로필 탭이 채워집니다")
        return {}
    with open(path, encoding='utf-8') as f:
...
def load_experts():
    ...
    P1/P2/P3 조사(2026-08-31~09-01)로 채워진 전문가 풀이다. 원본은 마크다운 표라
    코드 없이 못 고치지만(P2.md 자체를 편집), '표 안의 프로그램명/이름/전문가 정보'
    라는 단순한 구조만 걷어내는 얕은 파서라 P2.md 서식이 유지되는 한 안전하다.
    """
...
def research_date():
    """'조사를 언제 했는지' — 오늘 날짜가 아니라 조사 원본 파일이 실제로 갱신된 날짜.
    build_data.py 를 그냥 재실행(예: 프로필만 갱신)해도 이 값은 안 바뀐다 — 조사를
    다시 하면(_raw_synthesis1.json 이 새로 생성되면) 그때 자동으로 최신 날짜가 된다."""
    path = os.path.join(R, '_raw_synthesis1.json')
...
def build():
    ...
    out = {'generated_at': research_date(), 'project': 'CHOI'}
...
def write(out):
    ...
    print(f"    {'profile':20s} {'등록됨 (' + str(prof.get('name')) + ')' if prof.get('name') else '없음'}")
